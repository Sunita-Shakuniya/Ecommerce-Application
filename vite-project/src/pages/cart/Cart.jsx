...
function Cart() {
  ...
  useEffect(() => {
    const initFirebase = async () => {
      const { firedb, auth } = await initializeFirebase();
      setFiredb(firedb);
      setAuth(auth);
    };
  ...
  const gstamount = (totalAmount / 100) * 18;
  const gst = gstamount.toFixed(0);
  const grandtotal = gstamount + totalAmount;
  const grandTotal = grandtotal.toFixed(0);
  ...
  const buyNow = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:5000/api/create-order', {
        amount: grandTotal,
        currency: 'INR',
        receipt: 'order_rcptid_' + name,
      });
      
      const options = {
        key: data.key_id, // Using key_id from the backend
        amount: data.amount,
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
                ...
                <div className="justify-between mb-6 rounded-lg border  drop-shadow-xl bg-white p-6  sm:flex  sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div className="mt-5 sm:mt-0">
                      ...
                      <p className="mt-1 text-xs font-semibold text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{grandtotal}</p>
                    </div>
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
            ...
            <div className="flex justify-between mb-3">
              ...
              <div className>
                <p className="text-gray-700 font-bold " >₹{grandTotal}</p>
              </div>
            </div>
            {/* <Modal  /> */}
            <Modal 
